  ...
  async run (options) {
    ...
    if (help) {
      return buntstift.info(getUsage([
        { header: 'wolkenkit application import', content: this.description },
        { header: 'Synopsis', content: stripIndent`
          wolkenkit application import [--env <env>] --from=<directory> [--to-event-store]` },
    ...
    try {
      await wolkenkit.commands.application.import({
        directory,
        env,
        from,
        toEventStore
